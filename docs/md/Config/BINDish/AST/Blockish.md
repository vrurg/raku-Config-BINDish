...
Similar to `find-all(:$block!, ...)` candidate but makes makes sure that only one block entry is found. It either returns the block instance found, or [`Nil`](https://docs.raku.org/type/Nil), or throws with `Config::BINDish::X::Block::Ambiguous` if more than one candidate found.

### `multi find(:$option!, Bool :$local --> Config::BINDish::AST::Option)`

Similar to the candidate for `:$block!`, but for options. Returns [`Config::BINDish::AST::Option`](Option.md). Throws with `Config::BINDish::X::Option::Ambiguous`.
...
One of the key features of `get` which sets it apart of methods `value`, `option`, `block`, etc., is its ability to return some kind of default for a requested configuration element which is actually missing from the config file. For example, if we have a [`pre-declaration`](https://raku.land/?q=pre-declaration) like this:
